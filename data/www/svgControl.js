document.body.addEventListener("click", function (e) {
  if (
    e.target &&
    e.target.nodeName == "A" &&
    e.target.parentElement.className == "d-pad"
  ) {
...
let currentWidth;
let currentHeight;
function setSvgString(svgString, currentState) {
  resetTransform();
...
function makeTransformedSvgWithHeight() {
  ...
  transfromGroup.setAttribute(
    "transform",
    `matrix(${scaledAffine.join(", ")})`,
  );
...
function updateTransformText() {
  ...
  $("#transformText").text(
    `(${normalizeNumber(affineTransform[4] * 100)}, ${normalizeNumber(affineTransform[5] * 100)}) ${normalizeNumber(affineTransform[0])}x`,
  );
}
...
function convertJsonToDataURL(json, width, height) {
  $("#previewCanvas").remove();
  $(document.body).append(
    `<canvas id="previewCanvas" width="${width}" height="${height}" style="display: none;"></canvas>`,
  );
...
if (typeof module === "undefined") {
  window.svgControl = {
    ...
    getRenderScale,
    getRenderSvg,
    getCurrentSvgImageData,
    getSvgJson,
    convertJsonToDataURL,
  };
  console.log("svgControl exported to window");
}
